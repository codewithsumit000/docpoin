...
<body>
  ...
  <div class="sidebar">
    ...
    <div class="menu">
      ...
      <a href="../doctor/logout.php" class="logout">Logout</a>
    </div>
  </div>
<?php
session_start();
...
$app_count="SELECT COUNT(*) FROM appointments where doctor_id='$doctor_id'";
...
$today = date('Y-m-d');
$appointment_query = "
    SELECT a.*, d.name AS doctor_name, d.department,p.name AS patient_name
    FROM appointments a
    JOIN doctors d ON a.doctor_id = d.doctor_id
    JOIN patients p on a.patient_id = p.patient_id
    WHERE a.doctor_id='$doctor_id' AND a.appointment_date >= '$today'AND a.status != 'cancelled'
...
<div class="title">Welcome Back, DR <?php echo $result['name'] ;?>👋</div>
      ...
      <?php if(mysqli_num_rows($appointment_result) > 0): ?>
          <?php while($row = mysqli_fetch_assoc($appointment_result)): ?>
              <div class="appointment-item">
                  <span><?php echo $row['patient_name']; ?> — <?php echo $row['department']; ?> (<?php echo $row['appointment_date']; ?> <?php echo $row['appointment_time']; ?>)</span>
